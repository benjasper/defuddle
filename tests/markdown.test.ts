...
describe("Markdown conversion", () => {
	describe("links with spaces", () => {
		...
		test("should preserve existing parenthesis escaping for normal link destinations", () => {
			...
			expect(markdown).toContain(
				"[Open page](https://example.com/page\\(1\\))",
			);
		});
		...
		test("should preserve titles on link destinations containing spaces", () => {
			...
			expect(markdown).toContain(
				'[Open page](<myapp:open shared page> "Shared page")',
			);
		});
	});

	describe("linked images", () => {
		test("should not include block-level line breaks inside image links", () => {
			const markdown = createMarkdownContent(
				'<article><a href="https://example.com/gallery" title="Gallery"><div><figure><img src="https://example.com/image.jpg"></figure></div></a></article>',
				"https://example.com",
			);

			expect(markdown).toContain(
				'[![](https://example.com/image.jpg)](https://example.com/gallery "Gallery")',
			);
	...
	describe("base href resolution", () => {
		test("should resolve relative URLs against base href", async () => {
			...
			const result = await Defuddle(
				parseDocument(html, "https://arxiv.org/html/2312.00752"),
